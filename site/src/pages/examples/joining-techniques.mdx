...
import { countJoinAct } from "../../lib/scrolly-diagrams-more.mjs";
...
The next example counts with `COUNT(t.town)`, not `COUNT(*)`. After a `LEFT JOIN` those two give different answers for a county with no towns. Scroll to see why.
