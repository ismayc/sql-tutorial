...
import { aggregateAct, subqueryAct } from "../../lib/scrolly-diagrams-more.mjs";
...
`MIN` returns a number, not the row it came from. The next two examples recover the row by nesting one query inside another. Scroll to see the order the two queries run in.
