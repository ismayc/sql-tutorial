...
10. Count all distinct carriers in the flights table as `total_distinct_carriers`
...
25. Sort weather data by descending visibility and then descending wind speed
...
grade_this({
  ...
  # Enhanced order verification using data frame comparison for sorted columns
  ordered_solution <- .solution_ex25[order(.solution_ex25$visib, .solution_ex25$wind_speed, decreasing = TRUE), ]
  ordered_student_result <- .result_ex25[order(.result_ex25$visib, .result_ex25$wind_speed, decreasing = TRUE), ]
  if (!identical(ordered_student_result$visib, ordered_solution$visib) || !identical(ordered_student_result$wind_speed, ordered_solution$wind_speed)) {
...
grade_this({
  .solution_query28 <- "SELECT distance, CASE WHEN distance < 500 THEN 'Short' WHEN distance BETWEEN 500 AND 2000 THEN 'Medium' ELSE 'Long' END AS flight_distance_category FROM flights ORDER BY distance DESC;"
  ...
  # Verify the correctness of flight distance categorization
  if (!identical(.result_ex28$flight_distance_category, .solution_ex28$flight_distance_category)) {
    fail("The flight distance categorization in your query does not match the expected categories.")
  }
  
  # If order verification is required
  if (!identical(.result_ex28$distance, .solution_ex28$distance)) {
    fail("The order of distances in your query does not match the expected descending order.")
  }

  pass("Your query correctly categorizes and orders flights by distance as 'Short', 'Medium', or 'Long'.")
})
```
...
grade_this({
  ...
  sorted_avg_speed <- sort(.result_ex29$avg_speed, decreasing = TRUE)
  if (!identical(.result_ex29$avg_speed, sorted_avg_speed)) {
