...
12. Select airports above 40 latitude and in the America/New_York time zone.
...
grade_this({
  ...
  if (!all(.result_ex12$faa %in% .solution_ex12$faa)) {
...
13. Select airports (faa code and name) within 40 to 42 degrees latitude.
...
grade_this({
  ...
  # Validate the correctness of the result
  if (!all(.result_ex13$faa %in% .solution_ex13$faa)) {
...
14. Select airlines (carrier code and name) with name starting with an "A"
...
15. Select flights having 101 as the third, fourth, and fifth characters of the tail number.
...
-- SOLUTION: This query selects flights with tail numbers that have 'ar' as the third and fourth characters.
SELECT tailnum 
FROM flights 
WHERE tailnum LIKE '__101%';
```

```{r query15-check, echo=FALSE}
grade_this({
  # Execute the solution query to get the correct result for EXERCISE 15
  .solution_ex15 <- DBI::dbGetQuery(con, "SELECT tailnum FROM flights WHERE tailnum LIKE '__101%';")
  ...
  # Check if the result set matches the expected pattern
  if (!all(grepl('^__ar', .result_ex15$tailnum))) {
...
16. Select flights (all columns) where the carrier is either "AS" or "HA"
...
grade_this({
  ...
  pass("Your query correctly retrieves all flights where the carrier is either 'AS' or 'HA'.")
})
```

17. Select weather records (all columns) where wind direction is missing
...
18. Select weather records where temperature is not missing
...
19. Calculate the average temperature from the weather table as `avg_temperature`
...
20. Find the maximum arrival time for flights destined to ORD as `max_arr_time`
...
21. Find the first destination alphabetically for flights destined to SEA as `first_sea_dest`

```{sql query21, exercise=TRUE, exercise.eval=FALSE}
-- Write your SQL query here to find the first destination alphabetically 
-- for flights destined to SEA.
```
...
grade_this({
  .solution_query21 <- "SELECT MIN(dest) AS first_dest FROM flights WHERE dest = 'SEA';"
  .solution_ex21 <- DBI::dbGetQuery(con, .solution_query21)
  .result_ex21 <- DBI::dbGetQuery(con, .user_code)

  # Check for the 'first_dest' column in the student's result
  if (!"first_dest" %in% names(.result_ex21)) {
    fail("Your query is missing the 'first_dest' column.")
  }

  # Ensure the first destination alphabetically matches the solution
  if (.result_ex21$first_dest != .solution_ex21$first_dest) {
    fail("The first destination alphabetically returned by your query does not match the expected first destination.")
  }

  pass("Your query correctly finds the first destination alphabetically for flights destined to SEA.")
})
```

## Rounding summaries

22. Round wind gust values in the weather table to 0 decimal places as `rounded_wind_gust`
...
grade_this({
  ...
  if (!all.equal(.result_ex22$rounded_wind_gust, .solution_ex22$rounded_wind_gust, check.attributes = FALSE)) {
...
23. Sort flights by departure delay
...
grade_this({
  ...
  correct_order <- all(diff(.solution_ex23$dep_delay) >= 0)  # Check if departure delays are in ascending order
  student_order <- all(diff(.result_ex23$dep_delay) >= 0)

  if (!student_order || !identical(.result_ex23$dep_delay, .solution_ex23$dep_delay)) {
    fail("The flights in your query are not correctly ordered by departure delay.")
  }

  pass("Your query correctly sorts flights by departure delay.")
})
```

24. Sort flights by descending arrival delay
...
grade_this({
  ...
  correct_order <- all(diff(.solution_ex24$arr_delay, na.rm = TRUE) <= 0)  # Check if arrival delays are in descending order
  student_order <- all(diff(.result_ex24$arr_delay, na.rm = TRUE) <= 0)

  if (!student_order || !identical(.result_ex24$arr_delay[order(.result_ex24$arr_delay, decreasing = TRUE, na.last = NA)], .solution_ex24$arr_delay)) {
    fail("The flights in your query are not correctly ordered by descending arrival delay.")
  }

  pass("Your query correctly sorts flights by descending arrival delay.")
})
```

## Grouping

25. Sort weather data by descending visibility and wind speed
...
grade_this({
  ...
  # Check if the order of visibility and wind speed in the student's result matches the solution
  correct_order_visib <- all(diff(.solution_ex25$visib, na.rm = TRUE) <= 0)  # Check if visibility values are in descending order
  student_order_visib <- all(diff(.result_ex25$visib, na.rm = TRUE) <= 0)
  correct_order_wind_speed <- all(diff(.solution_ex25$wind_speed[order(.solution_ex25$visib, decreasing = TRUE)], na.rm = TRUE) <= 0)  # Check wind speed in descending order within each visibility group
  student_order_wind_speed <- all(diff(.result_ex25$wind_speed[order(.result_ex25$visib, decreasing = TRUE)], na.rm = TRUE) <= 0)

  if (!student_order_visib || !student_order_wind_speed || !identical(.result_ex25$visib, .solution_ex25$visib) || !identical(.result_ex25$wind_speed, .solution_ex25$wind_speed)) {
    fail("The weather data in your query are not correctly ordered by descending visibility and wind speed.")
  }

  pass("Your query correctly sorts weather data by descending visibility and wind speed.")
})
```

26. Group flights by origin and calculate the average arrival delay as `avg_arr_delay`
...
grade_this({
  .solution_query26 <- "SELECT origin, AVG(arr_delay) AS avg_arr_delay FROM flights GROUP BY origin ORDER BY avg_arr_delay DESC;"
  .solution_ex26 <- DBI::dbGetQuery(con, .solution_query26)
  .result_ex26 <- DBI::dbGetQuery(con, .user_code)

  # Check if the student's result matches the solution for 'origin' and 'avg_arr_delay'
  correct_origins <- identical(sort(unique(.result_ex26$origin)), sort(unique(.solution_ex26$origin)))
  correct_avg_arr_delays <- all(abs(.result_ex26$avg_arr_delay - .solution_ex26$avg_arr_delay[match(.result_ex26$origin, .solution_ex26$origin)]) < 1e-5)

  if (!correct_origins || !correct_avg_arr_delays) {
    fail("The average arrival delays by origin in your query do not match the expected results.")
  }
...
27. Group flights by destination and count the number of flights for each destination having more than 100 flights as `total_flights`
...
grade_this({
  ...
  # Check if the student's result matches the solution for 'dest' and 'total_flights'
  correct_dests <- identical(sort(unique(.result_ex27$dest)), sort(unique(.solution_ex27$dest)))
  correct_total_flights <- all(.result_ex27$total_flights[match(.result_ex27$dest, .solution_ex27$dest)] == .solution_ex27$total_flights)

  if (!correct_dests || !correct_total_flights) {
    fail("The count of flights by destination in your query does not match the expected results for destinations with more than 100 flights.")
  }
...
28. Categorize flights by distance as Short, Medium, or Long. Use 'Short' for distances less than 500 miles, 'Medium' for 500 to 2000 miles, and 'Long' for more than 2000 miles.
...
grade_this({
  .solution_query28 <- "SELECT CASE WHEN distance < 500 THEN 'Short' WHEN distance BETWEEN 500 AND 2000 THEN 'Medium' ELSE 'Long' END AS flight_distance_category FROM flights ORDER BY distance DESC;"
  .solution_ex28 <- DBI::dbGetQuery(con, .solution_query28)
  .result_ex28 <- DBI::dbGetQuery(con, .user_code)

  # Verify if the student's query categorization matches the solution
  correct_categories <- identical(.result_ex28$flight_distance_category, .solution_ex28$flight_distance_category)
...
29. Calculate the average speed of flights grouped by origin and destination and order by the fastest average speed
...
grade_this({
  ...
  # Check if the student's result has the expected columns
  ...
  if (length(missing_cols_ex29) > 0) {
    fail(glue::glue("Your query is missing the expected columns: {toString(missing_cols_ex29)}."))
  }
  
  if (length(extra_cols_ex29) > 0) {
    fail(glue::glue("Your query includes unexpected columns: {toString(extra_cols_ex29)}."))
  }
  
  # Check if the order of average speeds is correct
  correct_order <- all(diff(.result_ex29$avg_speed) <= 0)  # Check if all differences are non-positive, indicating descending order
  if (!correct_order) {
    fail("The average speeds in your query results are not in the correct descending order.")
  }
  
  # If all checks pass
  pass("Your query correctly calculates and orders the average speed of flights.")
})
```
...
30. Join flights (as `f`) with weather (as `w`) on `origin` and `time_hour` (returning only the rows matching between both tables). Select the `origin`, `dest`,
...
grade_this({
  ...
  if (length(missing_cols_ex30) > 0) {
    fail(glue::glue("Your query is missing the expected columns: {toString(missing_cols_ex30)}."))
  }
  
  if (length(extra_cols_ex30) > 0) {
    fail(glue::glue("Your query includes unexpected columns: {toString(extra_cols_ex30)}."))
  }
  
  # Check if the join is performed correctly by comparing the number of rows
  if (nrow(.result_ex30) != nrow(.solution_ex30)) {
    fail("The number of rows returned by your query does not match the expected result, indicating a potential issue with the join conditions.")
  }
  
  # If all checks pass
  pass("Your query correctly joins the flights table with the weather table on origin and time_hour.")
})
```

## LEFT JOIN

31. Left join flights with planes on tailnum (returning all rows from flights and only the rows from planes that match). Select the `tailnum`, `origin`, `dest`, `manufacturer`, and `model` fields.
...
grade_this({
  ...
  if (length(missing_cols_ex31) > 0) {
    fail(glue::glue("Your query is missing the expected columns: {toString(missing_cols_ex31)}."))
  }
  
  if (length(extra_cols_ex31) > 0) {
    fail(glue::glue("Your query includes unexpected columns: {toString(extra_cols_ex31)}."))
  }
  
  # Check if the left join is performed correctly by comparing the number of rows
  if (nrow(.result_ex31) < nrow(.solution_ex31)) {
    fail("The number of rows returned by your query is less than expected, indicating a potential issue with the left join conditions.")
  }
  
  # If all checks pass
  pass("Your query correctly left joins the flights table with the planes table on tailnum.")
})
```
...
grade_this({
  ...
  if (length(missing_cols_ex32) > 0) {
    fail(glue::glue("Your query is missing the expected columns: {toString(missing_cols_ex32)}."))
  }
  
  if (length(extra_cols_ex32) > 0) {
    fail(glue::glue("Your query includes unexpected columns: {toString(extra_cols_ex32)}."))
  }
  
  # Check if the right join is performed correctly by comparing the number of rows
  if (nrow(.result_ex32) < nrow(.solution_ex32)) {
    fail("The number of rows returned by your query is less than expected, indicating a potential issue with the right join conditions.")
  }
  
  # If all checks pass
  pass("Your query correctly right joins the airlines table with the flights table on carrier code, ordered by the airline's name in descending order.")
})
```

## Anti-Join

33. Anti-join flights (as `f`) with planes (as `p`) where plane's tailnum is missing. Select the `tailnum`, `origin`, and `dest` fields.
...
grade_this({
  ...
  if (length(missing_cols_ex33) > 0) {
    fail(glue::glue("Your query is missing the expected columns: {toString(missing_cols_ex33)}."))
  }
  
  if (length(extra_cols_ex33) > 0) {
    fail(glue::glue("Your query includes unexpected columns: {toString(extra_cols_ex33)}."))
  }
  
  # Check if the left join is performed correctly by comparing the number of rows where tailnum is missing
  if (nrow(.result_ex33) != nrow(.solution_ex33)) {
    fail("The number of rows returned by your query does not match the expected result, indicating a potential issue with the left join conditions or the WHERE clause.")
  }
  
  # If all checks pass
  pass("Your query correctly anti-joins (using a left join) the flights table with the planes table where the plane's tailnum is missing.")
})
```
...
-- Write your own query here determining a different result from these tables
```

```{sql query35, exercise=TRUE, exercise.eval=FALSE}
-- Write another query here determining a different result from these tables
```
