...
output: 
  learnr::tutorial:
    ...
    language:
      en:
        text:
          startover: "Reset All Exercises"
          areyousure: "Are you sure you want to reset all exercises? Your work will be lost."
...
/* Hide the section overview headers in main content */
.section.level1 > h1 {
  display: none !important;
}

/* Or if it's h2 headers causing the issue */
.section.level1 > h2 {
  display: none !important;
}
...
# Connect to the pre-built SQLite database file
# Using read-only mode for safety in multi-user environments
con <- DBI::dbConnect(
  RSQLite::SQLite(), 
  dbname = "pnw_flights_database.sqlite",
  flags = RSQLite::SQLITE_RO
)

# Load data frames for the ER diagram generation (if needed)
# These are read from the database to avoid needing separate RDA files
airlines <- DBI::dbReadTable(con, "airlines")
airports <- DBI::dbReadTable(con, "airports")
flights <- DBI::dbReadTable(con, "flights")
planes <- DBI::dbReadTable(con, "planes")
weather <- DBI::dbReadTable(con, "weather")
...
tutorial_options(exercise.cap = "Query", 
                 ...
                 exercise.timelimit = 60  # Give students 60 seconds for complex queries
)
```
...
$(document).on('shiny:connected', function() {
  setTimeout(function() {
    $('.btn-tutorial-run').removeClass('disabled').prop('disabled', false);
    $('.tutorial-exercise button').removeClass('disabled').prop('disabled', false);
  }, 1000);
});
")
```
...
<details open>
<summary style="display: inline-block; padding: 8px 16px; background-color: #5cb85c; color: white; border-radius: 4px; cursor: pointer; font-weight: bold;">Click to show/hide Notes and Tips</summary>

**Note: Query results display up to 500 rows in the output below to reduce website timeouts, but your query is validated against the complete dataset.**

**Tips:**

- Use "Hints" if you get stuck
- Click "Run Code" to execute your query
- When you feel confident about your code and results, click "Submit Answer" to check your work
- Your work should automatically be saved in your browser, but it's **STRONGLY** recommended to keep a separate backup of your queries in a text file elsewhere
</details>
...
13. Select the tail numbers (`tailnum`) of flights having 101 as the second, third, and fourth characters of the tail number.
...
16. Select weather records (all columns) where temperature is not missing
