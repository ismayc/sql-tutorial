...
*Scenario: You're sharing data with non-technical stakeholders who won't understand `land_area_sq_mi`. Rename columns to be clearer.*
...
*Scenario: When writing complex queries with multiple tables, short table aliases save typing and improve readability.*
...
### Unique 2: Which counties have towns that span multiple counties?

*Scenario: Some towns span county lines. Find which counties appear as secondary counties for any town.*
...
*Scenario: A journalist asks how comprehensive your database is. You need a quick count.*
...
*Scenario: You want to verify your data covers both Oregon and Washington (and only those states).*
...
### Counting 3: How many towns have incomplete data (missing secondary county info)?

*Scenario: Data quality check - how many towns don't span multiple counties?*
...
*Scenario: You're researching urban development and need to identify the larger municipalities.*
...
*Scenario: An Oregon-only grant targets cities over 50,000 people. Who qualifies?*
...
*Scenario: A regional initiative includes all Oregon towns plus major cities from Washington. The OR operator creates a union of conditions.*
...
*Scenario: A heritage tourism initiative targets counties established before 1860 that now have substantial populations (over 50,000).*
...
*Scenario: A "Main Street" revitalization program targets mid-sized towns. BETWEEN provides a clean way to specify ranges.*
...
*Scenario: Urban planners studying density want towns under 2 square miles.*
...
*Scenario: Find either (a) old Washington counties established before 1860, or (b) any Oregon county with population over 200,000. Parentheses control the logic.*
...
### Filtering 8: Find all counties named after presidents

*Scenario: For a history article, find counties whose etymology mentions "President".*
...
*Scenario: A linguistics researcher is studying town naming conventions.*
...
*Scenario: Identify coastal or river port towns for a maritime commerce study.*
...
### Filtering 11: Find counties with names starting with "King" or "Clark"

*Scenario: Using IN for multiple exact matches is cleaner than multiple OR conditions.*
...
*Scenario: Administrative complexity - which towns cross county boundaries?*
...
*Scenario: The opposite query - towns that don't span county lines.*
...
*Scenario: Default ORDER BY sorts ascending (smallest to largest).*
...
*Scenario: DESC reverses the order for "top N" style reports.*
...
*Scenario: Create a report organized by state, with largest cities first within each state.*
...
*Scenario: Historical timeline of county formation.*
...
*Scenario: Basic comparison of how many incorporated towns each state has.*
...
*Scenario: Which state has larger counties on average?*
...
*Scenario: Identify counties with many incorporated municipalities.*
...
*Scenario: Find only counties with 10 or more towns. HAVING filters after grouping (WHERE filters before).*
...
*Scenario: Which counties saw the most population growth in their towns from 2010 to 2020?*
...
*Scenario: Create size tiers for a stratified analysis. CASE WHEN acts like an IF-THEN statement.*
...
*Scenario: Identify growing and shrinking towns.*
...
*Scenario: Combine CASE WHEN with calculations to flag growth status.*
...
*Scenario: Classify towns as urban, suburban, or rural based on density.*
...
*Scenario: Categorize counties by historical period.*
